    ...
    <div className=' p-3 shadow-lg rounded-sm '>
        ...
        <Carousel
          ...
          {  props.images.map((data,index)=>
            <img className='w-full  sm:w-2/4 rounded-sm ' src={data} alt="" />
          )}
                            </Carousel>
            

            <div className='w-full sm:w-2/4 p-5 space-y-4 sm:space-y-10'>
                <h2 className='text-center font-mono font-bold text-lg'>{props.title}</h2>
                <div className="">
                ...
                <ul>
          {keys.map((key, index) => (
            <li key={index}>{key}: {props.technical[active][0][key]}</li>
          ))}
        </ul>
